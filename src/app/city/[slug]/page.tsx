...
export default async function CityPage({ params }: { params: { slug: string } }) {
  ...
  const [{ data: districts }, { data: companies }] = await Promise.all([
    supabase.from('districts').select('*').eq('city_id', city.id),
    supabase.from('companies').select('*, district:districts(*)').eq('city_id', city.id).order('is_premium', { ascending: false }).limit(20),
  ]);
  ...
  return (
    <div>
      <div className="bg-gradient-to-r from-orange-500 to-red-500 -mx-4 -mt-6 px-4 py-8 text-white mb-6">
        ...
        <p className="text-orange-100 mt-1">住人集装箱出租出售信息 · {districts?.length || 0}个区域</p>
      </div>
      ...
      {districts && districts.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
            {districts.map((d: any) => (
              <Link key={d.id} href={`/search?city=${params.slug}`}
      ...
      <h2 className="text-lg font-bold text-gray-900 mb-3">🏢 {city.name}公司 ({companies?.length || 0}家)</h2>
